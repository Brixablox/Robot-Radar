  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/signup', {
        ...
        body: JSON.stringify({ username, password, teamNumber: teamNumber ? parseInt(teamNumber) : undefined }),
      });
    ...
    <main style={{
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '8px', color: '#e2e8f0', fontWeight: 600 }}>Team Number (optional)</label>
            <input
              type="number"
              value={teamNumber}
        ...
        <p style={{ textAlign: 'center', marginTop: '24px', color: '#94a3b8', fontSize: '0.95rem' }}>
          ...
          <a href="/auth/login" style={{ color: '#67e8f9', textDecoration: 'none', fontWeight: 600 }}>
            Log in
          </a>
        </p>
      </div>
